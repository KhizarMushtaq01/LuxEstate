...
export function LoginPage({ agentLogin }) {
  const navigate = useNavigate();
  const location = useLocation();
  const { login, loading } = useAuthStore();
  const [form, setForm] = useState({ email: '', password: '' });
  const [show, setShow] = useState(false);
  const from = location.state?.from || (agentLogin ? '/agent' : '/');
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:block lg:w-1/2 relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-12">
          <Link to="/" className="flex items-center gap-2 mb-12">
            ...
            <div>
              <p className="font-display text-2xl text-gold-400">LuxEstate</p>
              <p className="text-white/40 text-xs tracking-widest -mt-1">REALTY GROUP</p>
            </div>
          </Link>
          <h2 className="font-display text-4xl text-white text-center mb-4">
            {agentLogin ? 'Agent Portal' : 'Welcome Back'}
          </h2>
          <p className="text-white/60 text-center text-lg leading-relaxed">
            Sign in to access your personalized dashboard, saved properties, and more.
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-cream">
        <div className="w-full max-w-md">
          ...
          <h1 className="font-display text-3xl text-navy-500 mb-2">
            {agentLogin ? 'Agent Sign In' : 'Sign In'}
          </h1>
          <p className="text-gray-400 text-sm mb-8">
            Don't have an account? <Link to="/register" className="text-gold-500 hover:underline">Register here</Link>
          </p>
...
export function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const path = user.role === 'agent' ? '/agent' : '/client';
      navigate(path);
    } catch (err) { toast.error(err.message); }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={form.firstName} onChange={e=>setForm({...form,firstName:e.target.value})} required className="input-field" placeholder="John" />
              </div>
              <div>
                <label className="label">Last Name</label>
                <input value={form.lastName} onChange={e=>setForm({...form,lastName:e.target.value})} required className="input-field" placeholder="Smith" />
              </div>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="tel" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} className="input-field" placeholder="(520) 555-0100" />
            </div>
            <div>
              <label className="label">Password</label>
              <div className="relative">
            ...
            <p className="text-xs text-gray-400 text-center">
              By registering, you agree to our <Link to="/terms" className="text-gold-500">Terms</Link> and <Link to="/privacy-policy" className="text-gold-500">Privacy Policy</Link>
            </p>
          </form>
        </div>
      </div>
