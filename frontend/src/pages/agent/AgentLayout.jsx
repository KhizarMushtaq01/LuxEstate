...
import { LayoutDashboard, Home, Calendar, MessageSquare, User, LogOut, ChevronRight, Plus, Menu } from 'lucide-react';
import { useState } from 'react';
import useAuthStore from '../../store/authStore';

const MENU = [
  { label: 'Dashboard', icon: LayoutDashboard, path: '/agent' },
  { label: 'My Listings', icon: Home, path: '/agent/properties' },
  { label: 'Appointments', icon: Calendar, path: '/agent/appointments' },
  { label: 'Leads', icon: MessageSquare, path: '/agent/leads' },
  { label: 'My Profile', icon: User, path: '/agent/profile' },
];
    ...
    <div className="flex flex-col h-full">
      <div className="p-6 border-b border-white/10">
        <Link to="/" className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-white/40 text-xs tracking-widest -mt-0.5">AGENT PORTAL</p>
          </div>
        </Link>
      </div>
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        {MENU.map(({ label, icon: Icon, path }) => {
          const active = location.pathname === path || (path !== '/agent' && location.pathname.startsWith(path));
          return (
      ...
      <div className="p-4 border-t border-white/10">
        <div className="px-2 mb-3">
          ...
          <p className="text-white/40 text-xs">Licensed Agent</p>
        </div>
        ...
        <header className="bg-white border-b border-gray-100 px-6 h-16 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-display text-lg text-navy-500">
              {MENU.find(m => location.pathname === m.path || (m.path !== '/agent' && location.pathname.startsWith(m.path)))?.label || 'Dashboard'}
            </h1>
          </div>
          <Link to="/agent/properties/new" className="btn-gold text-xs flex items-center gap-1.5">
